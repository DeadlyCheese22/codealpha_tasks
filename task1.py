...
try:
    ...
    sniff(prn=packet_callback, iface='Ethernet', store=0, count=10)
   # Initiates the packet sniffing process. It uses the sniff function from Scapy, specifying packet_callback as the callback function to handle captured packets. The iface parameter specifies the network interface to sniff on, in this case, it's set to 'Wi-Fi'. The store parameter is set to 0, which means it doesn't store packets in memory, and the count parameter is set to 10, which specifies the number of packets to sniff.
